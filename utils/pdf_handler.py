...
def convert_post_to_pdf(post):
    preamble_path = os.path.join(settings.BASE_DIR, "apps/posts/templates/posts/pdf_preamble.tex")
    ...
    cmd = [
        "pandoc",
        "-f", "markdown+all_symbols_escapable+raw_tex",
        "--pdf-engine=xelatex",
        "--standalone",
        "--number-sections",
        ...
        "-H", preamble_path,
        "-V", f"title={post.title}",
        "-V", f"author={author_name}",
        "-V", f"date={post.created_at.strftime('%Y-%m-%d')}",
        "--toc",
        "-o", pdf_path,
    ]
    ...
    try:
        ...
        if result.returncode != 0:
            error_msg = result.stderr.decode("utf-8", errors="ignore") if result.stderr else "Conversion failed"
